...
VanillaGridAggregate::VanillaGridAggregate(std::vector<Primitive> p) : primitives(std::move(p)) {
...
pstd::optional<ShapeIntersection> VanillaGridAggregate::Intersect(const Ray &ray, Float tMax) const {
...
bool VanillaGridAggregate::IntersectP(const Ray &ray, Float tMax) const {
 if (!bounds.IntersectP(ray.o, ray.d, tMax))
    return false;
...
VanillaGridAggregate *VanillaGridAggregate::Create(std::vector<Primitive> prims,
                                     const ParameterDictionary &parameters) {
  return new VanillaGridAggregate(std::move(prims));
}
